...
print(cocktails_max_ingredients)
```
...
ingredient_counts <- cocktailbar_new %>%
  count(ingredients) %>%  # Count occurrences of each ingredient
  filter(n == 1)  # Keep only ingredients that appear in one cocktail

# Step 2: Find the cocktails that contain those rare ingredients
cocktails_with_rare_ingredients <- cocktailbar_new %>%
  filter(ingredients %in% ingredient_counts$ingredients)  # Filter for cocktails with rare ingredients
...
one_cocktail_ingr <- cocktails_with_rare_ingredients %>%
  ...
  distinct(name)  # Ensure unique cocktail-ingredient pairs

# Print the result
print(one_cocktail_ingr)
...
cocktail_matrix <- matrix(NA, nrow = length(unique_cocktails), ncol = length(unique_ingredients))
...
for (i in 1:nrow(cocktailbar_new)) {
  cocktail_matrix[cocktailbar_new$name[i], cocktailbar_new$ingredients[i]] <- "X"
}
